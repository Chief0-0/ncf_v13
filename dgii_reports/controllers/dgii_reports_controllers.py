...
class DgiiReportsControllers(Controller):
    ...
    def redirect_link(self, ncf_rnc):
        ...
        if str(ncf_rnc)[:1] == 'B':
            invoice_id = env['account.move'].search([('reference', '=', ncf_rnc)], limit=1)
            if invoice_id:
                ...
                url = "%s/web#id=%s&action=%s&model=account.move&view_type=form" % (base_url, invoice_id.id, action.id)
